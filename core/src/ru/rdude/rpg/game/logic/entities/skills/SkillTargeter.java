...
public class SkillTargeter {
    ...
    public void target(Being<?> target) {
        ...
        if (currentTarget == Target.ALLY && !isAllies
            ...
            SkillUser.use(skillData, currentCaster, target);
            clear();
        }
    }
    ...
    private void findTargetForMonster(Being<?> caster, Target target, SkillData skillData) {
        if (target == Target.ANY) {
            SkillUser.use(skillData, caster, Target.RANDOM_ANY);
        } else if (target == Target.ANY_OTHER) {
            SkillUser.use(skillData, caster, Target.RANDOM_ANY_OTHER);
        } else if (target == Target.ALLY) {
            SkillUser.use(skillData, caster, Target.RANDOM_ALLY);
        } else if (target == Target.ENEMY) {
            SkillUser.use(skillData, caster, Target.RANDOM_ENEMY);
        }
    }
}
